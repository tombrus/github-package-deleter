...
## Build / run
Compile classpath (IntelliJ libraries, not in `lib/`):
- `lib/mvg-json-1.6.3.jar`
- `~/.m2/repository/com/formdev/flatlaf/3.0/flatlaf-3.0.jar`
- `~/.m2/repository/com/intellij/forms_rt/7.0.3/forms_rt-7.0.3.jar`

`makeMacApp.sh` needs `out/artifacts/github-package-deleter-jar/` built by IntelliJ first.
It jlinks a JDK 17 runtime, jpackages the app and copies it to the Desktop.
...
- The bundled runtime is jlinked with `--strip-native-commands`, so the app has no
  `bin/java`; use a separately installed JDK 17 to reproduce its behaviour.
